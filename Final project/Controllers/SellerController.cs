...
namespace Final_project.Controllers
{
    ...
    IFormFile[] newImageFiles, string[] Sizes, string[] newImageTypes)
        ...
        public async Task<IActionResult> CreateCategoryRequest(string CategoryName, string CategoryDiscription)
        {
            ...
            bool pending = await _unitOfWork.CategoryRequestRepository.GetAll()
                .AnyAsync(r => r.CategoryName.ToLower() == CategoryName.ToLower() && r.Status == "pending" && r.isDeleted == false);
            if (exists || pending)
            {
